...
/// declarations (and globally-registered examples in <see cref="VianaResultOptions"/>) into
/// <c>example</c> or <c>examples</c> entries on the relevant JSON response media types.
/// </summary>
public sealed class ResponseExampleOperationTransformer : IOpenApiOperationTransformer
{
    ...
    public Task TransformAsync(OpenApiOperation operation, OpenApiOperationTransformerContext context, CancellationToken cancellationToken)
    {
        ...
        var byStatus = ResponseExampleResolver.Collect(methodInfo, globals);
        if (byStatus.Count == 0)
            return Task.CompletedTask;
